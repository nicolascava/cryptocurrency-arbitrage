...
function prepareConfig(item, symbolCurrency, marketCurrency1, marketCurrency2) {
  ...
  const balance2 = balances[exchange2][marketCurrency2];
  const balance3 = balances[exchange3][symbolCurrency];
  ...
  const order2 = item[1][exchange2Pair].lowestAsk;
...
function findOrder2Quantity(
  ...
  const quantity = toFixed(order1QuantityAfterFees / order2.price, exchange2LotSize.decimals);
  const fees = disableFees ? 0 : toFixed(quantity * exchange2Fees, exchange2FeeDecimals);

  return {
    order2Quantity: quantity,
    order2QuantityAfterFees: quantity - fees,
  };
}

function findOrder3Quantity(order2QuantityAfterFees, exchange3LotSize) {
  return toFixed(order2QuantityAfterFees, exchange3LotSize.decimals);
}
...
function areOrdersInvalid(input) {
  ...
  if (
    ...
    (
      ...
      order2Quantity * order2.price > balance2 ||
...
// Chain A1
//
// buy ETH/BTC => buy BCH/ETH => sell BCH/BTC
export default async function (
  ...
  const { order2Quantity, order2QuantityAfterFees } = findOrder2Quantity(
    order1QuantityAfterFees,
    order2,
    exchange2Fees,
    exchange2FeeDecimals,
  ...
  const order3Quantity = findOrder3Quantity(order2QuantityAfterFees, exchange3LotSize, order3);
