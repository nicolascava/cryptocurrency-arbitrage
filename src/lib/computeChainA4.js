...
function prepareConfig(item, symbolCurrency, marketCurrency1, marketCurrency2) {
  ...
  const balance1 = balances[exchange1][symbolCurrency];
...
function findOrder2Quantity(
  ...
  const quantity = toFixed(quantityToTrade, exchange2LotSize.decimals);

  return {
    order2Quantity: quantity,
    order2QuantityCost: quantity * order2.price,
  };
}
...
function findOrder3Quantity(
  ...
  const fees = disableFees ? 0 : toFixed(order2QuantityCost * exchange3Fees, exchange3FeeDecimals);

  const quantityToTrade = order2QuantityCost + fees;
...
// Chain A4
//
// sell BCH/BTC => buy BCH/ETH => buy ETH/BTC
export default async function (
  ...
  const { order2Quantity, order2QuantityCost } = findOrder2Quantity(
    order1Quantity,
    exchange2LotSize,
    order2,
    exchange2Fees,
